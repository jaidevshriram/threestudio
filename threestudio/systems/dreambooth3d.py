...
@threestudio.register("dreambooth3d-system")
class DreamBooth3D(BaseLift3DSystem):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # Save the grid
        self.save_image_grid(
            f"it{self.true_global_step}-test/{batch['index'][0]}_grid.png",
            [
                {
                    "type": "rgb",
                    "img": out["comp_rgb"][0],
        ...
        # Save only the RGB
        self.save_image_grid(
            f"it{self.true_global_step}-test/{batch['index'][0]}_rgb.png",
            [
                {
                    "type": "rgb",
                    "img": out["comp_rgb"][0],
        ...
        self.save_data(
            f"it{self.true_global_step}-test/{batch['index'][0]}",
            {
                'elevation': batch['elevation'][0],
                'azimuth': batch['azimuth'][0],
            }
        )
